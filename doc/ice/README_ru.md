...
Эта плата потребляет больше энергии, чем типичный узел UAVCAN, поэтому она питается через дополнительный разъем.

**CAN-интерфейс**

Вы можете подключить эту плату к шине CAN с помощью одного из следующих разъемов:

1. UCANPHY Micro (JST-GH 4).
```
UAVCAN/CAN Physical Layer Specification Примечание.
Устройства, подающие питание на шину, должны обеспечивать напряжение 4,9-5,5 В на линии питания шины, номинальное напряжение 5,0 В.
...
Плата также имеет разъем SWD, предназначенный для обновления прошивки с помощью устройства [programmer-sniffer](doc/programmer_sniffer/README_ru.md).

**Входы и выходы**

1. Датчик топливного бака
2. Стартер
3. Искровое зажигание
...
![параметры](rpm_from_rc.png?raw=true "параметры")
